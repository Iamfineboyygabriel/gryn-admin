...
const InternationalYearOne = ({ onNext, applicationId }: any) => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  ...
  const [old_Level_Scratch_Card, setOld_Level_Scratch_Card] =
    useState<File | null>(null);
  ...
  const submitDocument = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const documents = [
        ...
        {
          type: DocumentType.OLD_LEVEL_SCRATCH_CARD,
...
export default InternationalYearOne;
