...
> = ({ sn, features, privileges, completed, onToggle, onPrivilegeToggle }) => (
  <tr className="border-b border-gray-200">
    ...
    <td className="py-5 px-6 text-sm whitespace-nowrap font-medium">
      {features}
    </td>
      ...
      <div className="flex flex-wrap gap-2">
        {privileges.map((privilege, index) => (
          ...
          >
            ...
            <p className="text-xs">{privilege.name}</p>
          </span>
        ))}
      </div>
    </td>
...
const Privileges: React.FC = () => {
  ...
  const [privilegesData, setPrivilegesData] = useState<FeaturePrivileges[]>([
    ...
    },
    {
      ...
      privileges: [{ name: "Send/Inbox", active: false }],
      completed: false,
    },
    {
      sn: 9,
  ...
  const handlePrivilegeToggle = useCallback(
    (feature: Features, privilegeName: string) => {
      setPrivilegesData((prev) =>
        prev.map((item) => {
          if (item.features === feature) {
  ...
  return (
    <div>
      ...
      <div className="overflow-hidden">
        ...
        {success && (
          <Alert severity="success" className="mt-4 mb-4">
            {success}
          </Alert>
        )}
