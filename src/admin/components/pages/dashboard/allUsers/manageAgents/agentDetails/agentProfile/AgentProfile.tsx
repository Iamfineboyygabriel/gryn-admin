...
} from "../../../../../../../../shared/redux/admin/slices/application.slices";
...
const AgentProfile: React.FC<AgentProfileProps> = ({
      ...
      try {
        ...
        if (response?.payload) {
          ...
          setFormData({
            ...
            bankDetails: {
              accountNumber:
                response?.payload?.bankAccounts?.[0]?.accountNumber || "",
              accountName:
                response?.payload?.bankAccounts?.[0]?.accountName || "",
  ...
  const updateAllDetails = async () => {
    ...
    try {
      if (
        Object.values(formData.personalDetails).some((value) => value !== "")
      ) {
        updates.push(
          dispatch(
            updateAgent({
            ...
            .then(() =>
              successMessages?.push("Personal details updated successfully"),
            )
            .catch(() =>
              errorMessages?.push("Failed to update personal details"),
            ),
        );
      }
      ...
      if (Object.values(formData?.bankDetails)?.some((value) => value !== "")) {
        updates.push(
          dispatch(
            ...
            .then(() =>
              successMessages?.push("Bank details updated successfully"),
            )
            .catch(() => errorMessages?.push("Failed to update bank details")),
        );
      }

      const documentUpdates = Object?.entries(files)?.map(
            ...
            return updateRegistrationUploadedDocument(endpoint, form)
              .then((response) => {
                if (response?.status === 201) {
                  ...
                  errorMessages?.push(
                    `Failed to update document ${documentType}`,
                  );
                }
              })
              .catch(() =>
  ...
  const renderInput = (
    ...
    <input
      ...
      value={
        editMode
          ? formData[section][name as keyof (typeof formData)[typeof section]]
          : value || ""
      }
      onChange={(e) =>
        editMode && handleInputChange(section, name, e.target.value)
      }
      readOnly={!editMode}
                    ...
                    >
                      <div className="flex justify-between">
                        <div className="flex items-center gap-5">
                          <div className="flex gap-2">
                            ...
                            <p className="text-lg truncate max-w-[170px] whitespace-nowrap font-light">
                              {selectedFileNames[doc?.documentType] ||
                                doc?.name ||
                                "No file uploaded"}
                            </p>
                          </div>
                        ...
                        <input
                          ...
                          id={doc.documentType}
                          onChange={(e) =>
                            handleFileChange(e, doc.documentType)
                          }
                          disabled={!editMode}
                          className="hidden"
                        />
                        <button.PrimaryButton
                          ...
                          onClick={() =>
                            document.getElementById(doc.documentType)?.click()
                          }
                          disabled={!editMode}
                        >
