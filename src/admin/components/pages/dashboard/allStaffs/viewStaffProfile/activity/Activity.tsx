...
}> = ({ activity, isEven, showOnlineIndicator }) => (
  <div
    className={`px-[2em] py-4 ${
      isEven ? "bg-[#F7F7F7]" : "bg-white"
...
const Activity: React.FC<{ staffEmail: any | null }> = ({ staffEmail }) => {
  ...
  const isUserCurrentlyOnline = useCallback((activities: any[]) => {
    ...
    return (
      mostRecentActivity?.activity === "Login User" &&
      isToday(mostRecentActivity?.createdAt)
    );
  }, []);
  ...
  const shouldShowEmptyState =
    !loading && activities?.length === 0 && page === 1;
  const isOnline = isUserCurrentlyOnline(activities);
    ...
    <main className="font-outfit divide-y divide-gray-100">
      {activities.map((activity: any, index: number) => (
        <ActivityItem
          ...
          isEven={index % 2 === 0}
          showOnlineIndicator={
            isOnline && index === 0 && activity?.activity === "Login User"
          }
        />
      ))}
