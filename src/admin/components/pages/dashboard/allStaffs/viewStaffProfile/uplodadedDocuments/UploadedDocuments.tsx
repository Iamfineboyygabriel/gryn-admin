        ...
        Object.entries(files).map(async ([documentType, file]) => {
          ...
          if (file && doc) {
            try {
              ...
              const response = await updateStaffRegistrationDocument(endpoint, form);
          ...
          <div className="mt-[2em] grid w-[85%] grid-cols-2 gap-10">
            {documents.map((doc: DocumentType) => (
              <div key={doc?.id}>
                ...
                <div
                  className={`dark:border-border mt-2 flex items-center justify-between rounded-lg border-[1px] border-gray-300 px-[1.5em] py-2 dark:bg-gray-700 ${
