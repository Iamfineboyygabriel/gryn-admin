...
const Transaction = () => {
  ...
  const renderTransactionTableBody = useCallback(() => {
    if (loading) {
      return Array.from({ length: 10 }).map((_, index) => <SkeletonRow key={index} />);
    }
    ...
    return filteredAndSortedSalaries.map((item: SalaryItem, index: number) => (
      ...
      >
        ...
        <td className="whitespace-nowrap px-3 py-2">
          <button
            className={`mr-2 rounded-full px-3 text-sm py-1 text-white ${getStatusColor(item?.status)}`}
          >
            {item?.status}
          </button>
        </td>
  ...
  }, [filteredAndSortedSalaries, currentPage, itemsPerPage, formatData, loading]);
      ...
      <div className="flex justify-between gap-[1em]">
        <div className="h-[300px] w-full rounded-lg bg-white px-[2.5em] py-[1.3em] overflow-y-auto">
          ...
          <table className="w-full table-auto">
            ...
            <tbody className="overflow-y-auto">{renderTransactionTableBody()}</tbody>
          </table>
        </div>
