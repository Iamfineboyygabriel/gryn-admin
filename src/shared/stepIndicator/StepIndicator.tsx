...
const StepIndicator: React.FC<StepIndicatorProps> = ({
  currentStep,
  completedSteps,
  customLabels = [], // Default to an empty array if no custom labels are passed
}) => {

  const defaultLabels = ["Personal Details", "Degree", "Upload Document"];

  const steps: Step[] = defaultLabels.map((defaultLabel, index) => ({
    step: index + 1,
    label: customLabels[index] || defaultLabel, 
  }));
