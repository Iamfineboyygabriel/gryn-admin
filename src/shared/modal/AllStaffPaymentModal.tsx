...
const AllStaffPaymentModal = ({ isOpen, onClose, payment }: any) => {
  ...
  const hasDocuments =
    payment?.invoice?.document && payment.invoice.document.length > 0;

  return (
    <main
      className={`fixed font-outfit inset-y-0 py-2 right-0 w-[500px] bg-white shadow-lg transform ${
    ...
    >
      <div className="h-full flex flex-col">
        ...
        <div className="flex-grow overflow-y-auto px-[1.5em]">
          ...
          {hasDocuments &&
            payment.invoice.document.map((doc: any, index: number) => (
              <div
                key={doc.id}
                className="bg-gray-100 flex mt-[2em] px-2 items-center justify-between py-2"
              >
