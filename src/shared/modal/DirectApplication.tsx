...
import {
  findSchoolByCountryName,
  findSchoolLinkByCountryAndUniversity,
} from "../redux/admin/services/application.services";
...
const DirectApplication: React.FC<DirectApplicationProps> = ({
  addSchoolLink,
}) => {
  ...
  const fetchUniversities = async (country: string) => {
    ...
    try {
      ...
      if (response?.status === 200 && response?.data) {
        ...
        setError(
          response?.message || "Failed to fetch universities. Please try again."
        );
      }
    } catch (error: any) {
  ...
  const handleContinue = async () => {
    ...
    try {
      const endpoint = `/school/SchoolLinkByCountryAndUniversity?country=${encodeURIComponent(
        country
      ...
      {!showApplicationLinks ? (
        <div className="m-auto w-[24em] py-[1em] text-center">
          <header className="flex flex-col">
            ...
            <p className="font-light tracking-wide space-x-2 mt-[1.1em]">
          ...
          <Link to={addSchoolLink}>
            <span className="text-pink-primary font-medium items-center mb-2 cursor-pointer mt-[8px] flex justify-center">
              <GoPlus className="text-pink-primary size-8 mr-2" />
              Add New School
            </span>
          </Link>
        </div>
