...
const NewBudgets: React.FC = () => {
  ...
  const handleDropdownChange =
    (key: "paymentType" | "location") => (item: DropdownItem | null) => {
  ...
  const calculateTotalRate = (): number => {
    return formData.BudgetItem.reduce(
      (sum, item) => sum + (typeof item.rate === "number" ? item.rate : 0),
      0
    );
  };
  ...
  return (
    <main className="font-outfit">
      ...
      <form
        ...
        className="mt-4 flex flex-col gap-5 h-auto w-full overflow-auto rounded-lg bg-white p-3 pb-10"
      >
        ...
        <div className="w-[42%] flex flex-col gap-5">
          <Dropdown
            ...
            items={paymentTypes}
            selectedItem={
              paymentTypes.find((item) => item.name === formData.paymentType) ||
              null
            }
          ...
          <Dropdown
            ...
            items={branches}
            selectedItem={
              branches.find((item) => item.name === formData.location) || null
            }
        ...
        <div className="flex flex-col gap-4">
          ...
          {formData.BudgetItem.map((item, index) => (
            <div
              key={index}
              className="flex lg:flex-row flex-col lg:justify-between items-start gap-4"
            >
              <div className="lg:w-1/4 w-full">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleBudgetItemChange(index, "productName", e.target.value)
                  }
                  className="mt-2 w-full rounded-lg border-2 p-3 focus:outline-none"
                />
              </div>
              <div className="flex lg:flex-row flex-col w-full lg:w-3/4 gap-4">
                {["quantity", "rate", "amount", "discount"].map((field) => (
                  <div key={field} className="lg:w-1/4 w-full">
                    <label
                      htmlFor={`${field}-${index}`}
          ...
          <div className="w-40">
            <label htmlFor="totalRate" className="block font-medium">
              Total Rate
            </label>
            <input
              id="totalRate"
