...
const UpdatePayment = () => {
  ...
  const state = location.state as LocationState;
   ...
   const [documents, setDocuments] = useState<Record<DocumentType, DocumentState>>({
    [DocumentType.SERVICE_CHARGE]: { id: null, file: null, status: "PENDING", canUpdate: false, originalName: "" },
    [DocumentType.APPLICATION_FEE]: { id: null, file: null, status: "PENDING", canUpdate: false, originalName: "" },
    [DocumentType.TUTION_FEE]: { id: null, file: null, status: "PENDING", canUpdate: false, originalName: "" }
  });
  const [loading, setLoading] = useState(false);
  const [isModalOpen, setModalOpen] = useState(false);
  const [errors, setErrors] = useState<Record<DocumentType, string>>({} as Record<DocumentType, string>);
  const [successes, setSuccesses] = useState<Record<DocumentType, boolean>>({} as Record<DocumentType, boolean>);
  ...
  const setFileForDocument = (documentType: DocumentType) => (file: SetStateAction<File | null>) => {
    if (documents[documentType].canUpdate) {
      setDocuments(prev => ({
        ...prev,
        [documentType]: {
  ...
  const submitPayment = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const uploadPromises = Object.entries(documents)
        .filter(([_, docState]) => docState.canUpdate && docState.file && docState.id)
  ...
  const canSubmit = Object.values(documents).some(doc => 
    doc.canUpdate && doc.file !== null
  );
        ...
        <div className="w-[26em]">
          <div className="mt-[2em]">
            ...
            <form onSubmit={submitPayment}>
              <div className="mt-[1.5em] grid w-[230%] grid-cols-2 gap-4">
