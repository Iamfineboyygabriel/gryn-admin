...
import {
  ...
  submitBankDetails,
  updateBankDetails,
} from "../../../../../../shared/redux/admin/services/application.services";
...
const BankDetails = () => {
  const { userProfile, loading: accountLoading } = useUserProfile();
  ...
  useEffect(() => {
    if (userProfile?.user?.bankAccounts) {
      let bankAccount: BankAccount;

      if (Array?.isArray(userProfile?.user?.bankAccounts)) {
        bankAccount = userProfile?.user?.bankAccounts[0];
      } else {
        bankAccount = userProfile?.user?.bankAccounts as BankAccount;
      }
  ...
  return (
    <main>
      ...
      <div className="flex flex-col mt-[2em] w-full lg:w-[40%] gap-[1.5em]">
        <Dropdown
          label="Bank"
          items={dropdownItems}
